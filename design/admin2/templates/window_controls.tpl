...
<ul class="tabs{if $tabs_disabled} disabled{/if}{if $read_open_tab_by_cookie} tabs-by-cookie{/if}">
    ...
    {* Ordering *}
    <li id="node-tab-ordering" class="{if $node_tab_index|eq('ordering')} selected{/if}">
        {if $tabs_disabled}
    ...
    {* View Extras *}
    <li id="node-tab-view-extras" class="last{if $node_tab_index|eq('view_extras')} selected{/if}">
        {if $tabs_disabled}
            <span class="disabled" title="{'Tab is disabled, enable with toggler to the left of these tabs.'|i18n( 'design/admin/node/view/full' )}">{'View Extras'|i18n( 'design/admin/node/view/full' )}</span>
        {else}
            <a href={concat( $node_url_alias, '/(tab)/view_extras' )|ezurl} title="{'Show view_extra values.'|i18n( 'design/admin/node/view/full' )}">{'View Extras'|i18n( 'design/admin/node/view/full' )}</a>
        {/if}
    </li>
</ul>
